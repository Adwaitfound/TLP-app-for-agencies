...
export async function logAuditEvent(data: AuditLogData) {
  ...
  const auditDisabled =
    process.env.NEXT_PUBLIC_AUDIT_DISABLED === "true" ||
  ...
  try {
    ...
    const sanitizedDetails = data.details
      ? JSON.parse(JSON.stringify(data.details).substring(0, 10000))
      : null;
...
async function syncToGoogleSheets(logData: any) {
  ...
  try {
    ...
    if (!appendResponse.ok) {
      ...
      throw new Error(
        `Google Sheets API error: ${error.error?.message || appendResponse.statusText}`,
      );
    }
