...
export async function createClientAccount(
  ...
  try {
    console.log("[SERVER] createClientAccount called with:", {
    ...
    if (userExists) {
      console.log(
        "[SERVER] Auth user already exists, finding existing user ID...",
      );
      const {
        data: { users },
        error: listError,
      ...
      if (listError) {
        console.error("[SERVER] Failed to list auth users:", listError);
        return {
          success: false,
          error: "Failed to retrieve existing auth user",
        };
      }
      ...
      if (!existingAuthUser?.id) {
        return {
          success: false,
          error: "User exists but could not retrieve ID",
        };
      }
    ...
    if (clientError) {
      ...
      return {
        success: false,
        error: `Client insert error: ${clientError.message}`,
      };
    }
