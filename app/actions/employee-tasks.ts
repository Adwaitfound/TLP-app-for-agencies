...
export async function createTask(data: {
  ...
  console.log(
    "[createTask] Creating task for user:",
    user.id,
    user.email,
    "Data:",
    data,
  ...
  if (!data.project_id && data.proposed_project_name) {
    ...
    if (
      data.proposed_project_vertical &&
      data.proposed_project_vertical.trim()
    ) {
...
export async function deleteTask(taskId: string) {
  ...
  console.log(
    "[deleteTask] Attempting delete from employee_tasks where id=" +
      taskId +
      " AND user_id=" +
      user.id,
  );
...
export async function getTodayTasks() {
  ...
  const data = Array.from(taskMap.values()).sort((a, b) => {
  ...
  console.log(
    "[getTodayTasks] Query result: active tasks:",
    activeTasks?.length,
    "proposal tasks:",
...
export async function getPendingProjectProposals() {
  ...
  const { data, error } = await supabase
    .from("employee_tasks")
    .select(
      "id, title, description, proposed_project_name, proposed_project_vertical, proposed_project_status, proposed_project_notes, user_id, created_at, users:users!employee_tasks_user_id_fkey(full_name, email), project_id, projects(name)",
    )
...
export async function reviewProjectProposal(args: {
  ...
  console.log(
    "[reviewProjectProposal] 🔵 START - Decision:",
    args.decision,
    "TaskID:",
    args.taskId,
    "ProjectID:",
...
export async function debugAllUserTasks() {
  ...
  const { data, error } = await supabase
    .from("employee_tasks")
    .select(
      "id, title, status, proposed_project_name, proposed_project_status, user_id, created_at",
    )
    .eq("user_id", user.id)
    .order("created_at", { ascending: false });

  console.log("[debugAllUserTasks] ALL tasks for user", user.id, ":", {
    count: data?.length,
    data,
    error,
  });
