  ...
  try {
    ...
    if (decision === "approved" && userData.role === "client") {
      ...
      if (!clientFetchError && !existingClient) {
        const companyName =
          userData.company_name || userData.full_name || userData.email;
        ...
        if (insertError) {
          return {
            success: false,
            error: `Client creation failed: ${insertError.message}`,
          };
        }
      }
    }
    ...
    return {
      success: false,
      error: err?.message || "Unexpected error approving user",
    };
  }
}
