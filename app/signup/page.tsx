  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (role === "client") {
        ...
        if (!res.success)
          throw new Error(res.error || "Failed to register pending user");
        debug.success(
          "SIGNUP",
          "Pending client profile stored via server action",
          { email },
        );
      } else {
      ...
      if (role === "admin") {
        ...
        setError(
          "Account created. Pending admin approval — you will be able to sign in once approved.",
        );
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <div
            className={`mx-auto mb-4 w-16 h-16 rounded-full bg-gradient-to-br ${config.color} p-0.5`}
          >
        ...
        <CardContent>
          ...
          >
            ...
            <Button type="submit" disabled={loading} className="w-full">
              {loading
                ? "Creating Account..."
                : `Sign Up as ${config.title.replace(" Account", "")}`}
            </Button>
