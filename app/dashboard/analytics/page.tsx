  ...
  async function fetchAnalytics() {
    ...
    try {
      ...
      const stats: ServiceStats[] = [
        ...
        const serviceProjects = projectsData.filter(
          (p) => p.service_type === service,
        );
        const serviceProjectIds = serviceProjects.map((p) => p.id);
        const serviceInvoices = invoicesData.filter(
          (inv) =>
            serviceProjectIds.includes(inv.project_id) && inv.status === "paid",
        );
        const totalRevenue = serviceInvoices.reduce(
          (sum, inv) => sum + (inv.total || 0),
          0,
        );
        const activeProjects = serviceProjects.filter(
        ...
        return {
          ...
          avg_project_value:
            serviceProjects.length > 0
              ? totalRevenue / serviceProjects.length
              : 0,
        };
      });
    ...
    <div className="flex flex-col gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ₹
              {totalProjects > 0
                ? Math.round(totalRevenue / totalProjects).toLocaleString()
                : 0}
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Active vs completed projects by service
          </CardDescription>
        </CardHeader>
        <CardContent>
          {serviceStats.some(
            (s) => s.active_projects > 0 || s.project_count > 0,
          ) ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart
                data={serviceStats.map((s) => ({
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Detailed performance metrics by service vertical
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-6">
            {serviceStats.map((stat) => {
              ...
              const revenuePercent =
                totalRevenue > 0
                  ? (stat.total_revenue / totalRevenue) * 100
                  : 0;

              return (
                <div
                  key={stat.service_type}
                  className="space-y-3 border-b last:border-b-0 pb-6 last:pb-0"
                >
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      className={
                        serviceConfig.bgColor + " " + serviceConfig.textColor
                      }
                    >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <div className="font-semibold text-lg">
                        ₹{Math.round(stat.avg_project_value).toLocaleString()}
                      </div>
                    </div>
                    <div>
