...
export default function AdminDashboard() {
      ...
      try {
        ...
        const avgProjectValue =
          projectsData && projectsData.length > 0
            ? totalProjectBudget / projectsData.length
            : 0;
        ...
        setError(
          "Something went wrong while loading your dashboard. Please refresh.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const getFilteredStats = () => {
    ...
    const avgProjectValue =
      filteredProjects.length > 0
        ? totalProjectBudget / filteredProjects.length
        : 0;
  ...
  const getTotalProjectAmount = (): number => {
    return filterByTimePeriod(projects).reduce(
    ...
    <div className="flex flex-col gap-4 md:gap-6">
      ...
      <Dialog open={devToolsOpen} onOpenChange={setDevToolsOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Grant admin or confirm email for a user
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-2">
              <label htmlFor="grant-admin-email" className="text-sm">
            ...
            <p className="text-xs text-muted-foreground">
              Hint: double-click the “Dashboard” title to open this.
            </p>
          </div>
        </DialogContent>
      </Dialog>
        ...
        <CardContent className="space-y-3">
          {pendingUsersLoading ? (
            ...
            pendingUsers.map((pendingUser) => (
              <div
                key={pendingUser.id}
                className="p-3 border rounded-lg flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"
              >
                ...
                <div className="flex gap-2 flex-wrap">
                  <Button
                    ...
                    onClick={() =>
                      handleUserApproval(pendingUser.id, "approved")
                    }
                  >
                    Approve
                  </Button>
                  ...
                  <Button
                    ...
                    disabled={pendingActionUserId === pendingUser.id}
                    onClick={() =>
                      handleUserApproval(pendingUser.id, "rejected")
                    }
                  >
                    Reject
                  </Button>
      ...
      <Card>
        <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div>
            <CardTitle className="text-lg md:text-xl">
              Project Proposals from Employees
            </CardTitle>
            <CardDescription className="text-xs md:text-sm">
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      ...
                      {proposal.projects?.name && (
                        <p className="text-xs text-muted-foreground">
                          Currently mapped: {proposal.projects.name}
                        </p>
                      )}
                    </div>
                    <div className="w-48">
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={() =>
                        handleProposalDecision(proposal.id, "approved")
                      }
                    >
                      Approve
                    </Button>
                    <Button
                      ...
                      disabled={proposalSubmitting}
                      onClick={() =>
                        handleProposalDecision(proposal.id, "rejected")
                      }
                    >
                      Reject
                    </Button>
          ...
          <CardContent>
            <Tabs defaultValue="video_production" className="w-full">
              ...
              <TabsContent value="video_production">
                <div className="space-y-3">
                  {projects.filter((p) => p.service_type === "video_production")
                    .length === 0 ? (
                    <p className="text-sm text-muted-foreground">
                      No video production projects yet
                    </p>
                  ) : (
                    projects
              ...
              <TabsContent value="social_media">
                <div className="space-y-3">
                  {projects.filter((p) => p.service_type === "social_media")
                    .length === 0 ? (
                    <p className="text-sm text-muted-foreground">
                      No social media projects yet
                    </p>
                  ) : (
                    projects
              ...
              <TabsContent value="design_branding">
                <div className="space-y-3">
                  {projects.filter((p) => p.service_type === "design_branding")
                    .length === 0 ? (
                    <p className="text-sm text-muted-foreground">
                      No design & branding projects yet
                    </p>
                  ) : (
                    projects
              ...
              <div className="space-y-3">
                {recentClients.map((client) => {
                  ...
                  const clientServices = [
                    ...new Set(clientProjects.map((p) => p.service_type)),
                  ];
