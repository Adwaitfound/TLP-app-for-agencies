...
export default function ClientProjectPage({
    ...
    async function fetchData() {
      ...
      try {
        ...
        const { data: commentsData } = await supabase
          ...
          .select("*, assignee:users(id, email, full_name)")
          .eq("project_id", projectId)
          .order("created_at", { ascending: false });
        ...
        const { data: teamData } = await supabase
          ...
          .eq("project_id", projectId);
        const members = (teamData || []).flatMap((t: any) => {
          const u = t.users;
          if (!u) return [];
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      setCommentError(
        (e as any)?.message || "Unexpected error submitting comment",
      );
    } finally {
      setCommentSubmitting(false);
    }
  ...
  const assignComment = async (commentId: string, userId: string) => {
    ...
    if (!error && data) {
      setComments((prev) =>
        prev.map((c) =>
          c.id === commentId
            ? { ...c, assigned_user_id: userId, assignee: data.assignee }
            : c,
        ),
      );
    }
        ...
        <div className="space-y-6">
              ...
              {restrictAccess && (
                <div className="p-3 rounded-md bg-destructive/10 border border-destructive/20 mb-4 flex items-center gap-2">
                  ...
                  <p className="text-sm text-destructive">
                    Access restricted: invoice overdue by 30+ days. Please
                    settle dues to continue.
                  </p>
                </div>
              )}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Leave change requests or feedback. Admins are notified.
              </CardDescription>
            </CardHeader>
                        ...
                        teamMembers.length > 0 && (
                          <div className="flex items-center gap-2 pt-1">
                            <select
                              ...
                              value={
                                assignees[c.id] ?? c.assigned_user_id ?? ""
                              }
                              onChange={(e) =>
                                setAssignees((prev) => ({
                                  ...prev,
                            ...
                            <Button
                              ...
                              onClick={() =>
                                assignees[c.id] &&
                                assignComment(c.id, assignees[c.id])
                              }
                            >
                              Assign
                            </Button>
            ...
            <CardContent>
              {project.client_approved ? (
                <div className="flex items-center gap-2 text-green-600">
                  <CheckCircle2 className="h-5 w-5" /> Approved on{" "}
