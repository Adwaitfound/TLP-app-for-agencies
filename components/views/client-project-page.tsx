...
export default function ClientProjectPage({ projectId }: ClientProjectPageProps) {
    ...
    const submitComment = async () => {
        ...
        try {
            ...
            const { data, error } = await supabase
                .from('project_comments')
                .insert({ project_id: projectId, author_user_id: user!.id, text: trimmed })
                .select('*')
                .single()
    ...
    const assignComment = async (commentId: string, userId: string) => {
        ...
        const { data, error } = await supabase
            ...
            .single()
        if (!error && data) {
            setComments(prev => prev.map(c => c.id === commentId ? { ...c, assigned_user_id: userId, assignee: data.assignee } : c))
        }
    }
