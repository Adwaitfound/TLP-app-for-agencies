...
import type { VendorPayment, VendorProjectAssignment, ServiceType } from "@/types"
...
export function ProjectBudgetTracker({ payments, assignments }: ProjectBudgetTrackerProps) {
    ...
    // Group by vertical
    const videoProduction = projects.filter(p => p.service_type === 'video_production')
    const socialMedia = projects.filter(p => p.service_type === 'social_media')
    const designBranding = projects.filter(p => p.service_type === 'design_branding')

    const renderProjectGroup = (title: string, projectList: ProjectBudget[], serviceType: ServiceType) => {
        ...
        const totalPending = projectList.reduce((sum, p) => sum + p.pendingPayments, 0)
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="grid gap-4 md:grid-cols-3">
                        ...
                        <div className="rounded-lg border p-3">
                            ...
                            <p className="text-xs text-muted-foreground mt-1">
                                {(((totalBudget - totalSpent - totalPending) / totalBudget) * 100).toFixed(1)}% available
                            </p>
                        </div>
                    </div>
                                ...
                                <div key={project.id} className="rounded-lg border p-4 space-y-3">
                                    ...
                                    <div className="space-y-1">
                                        <div className="flex justify-between text-sm">
                                            <span className="text-muted-foreground">Spent & Pending</span>
                                            <span className="font-medium">
                                                {formatCurrency(spent + pending)} ({(percentSpent + percentPending).toFixed(1)}%)
                                            </span>
                                        </div>
                                        <Progress
                                    ...
                                    <div className="grid grid-cols-3 gap-2 text-sm">
                                        <div>
                                            ...
                                            <p className="font-medium text-red-600">{formatCurrency(spent)}</p>
                                        </div>
                                        <div>
                                            <p className="text-muted-foreground">Pending</p>
                                            <p className="font-medium text-yellow-600">{formatCurrency(pending)}</p>
                                        </div>
                                        <div>
    ...
    return (
        <div className="space-y-4">
            {renderProjectGroup("Video Production", videoProduction, 'video_production')}
            {renderProjectGroup("Social Media", socialMedia, 'social_media')}
            {renderProjectGroup("Design & Branding", designBranding, 'design_branding')}
