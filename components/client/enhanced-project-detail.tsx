...
export default function EnhancedClientProjectDetail({
  projectId,
  ...
  useEffect(() => {
    async function fetchProjectData() {
      ...
      try {
        ...
        if (teamErr)
          console.error("Error fetching team:", teamErr.message || teamErr);
        ...
        if (filesErr)
          console.error("Error fetching files:", filesErr.message || filesErr);
        setFiles(filesData || []);
        ...
        if (milestonesErr)
          console.error(
            ...
            milestonesErr.message || milestonesErr,
          );
        ...
        if (subProjectsErr)
          console.error(
            "Error fetching sub-projects:",
            subProjectsErr.message || subProjectsErr,
          );
        ...
        if (commentsErr)
          console.error(
            ...
            commentsErr.message || commentsErr,
          );
  ...
  const deliverablesStats = useMemo(() => {
    ...
    const completed = files.filter(
      (f) => f.status === "approved" || f.status === "delivered",
    ).length;
  ...
  const todaysActivity = useMemo(() => {
    ...
    const todayComments = comments.filter(
      (c) => new Date(c.created_at) >= today,
    );
    const todayTasks = subProjects.filter(
      (t) => new Date(t.updated_at || t.created_at) >= today,
    );
      ...
      const { error } = await supabase.from("sub_projects").insert({
        ...
        metadata: {
          ...
          estimated_hours: taskFormData.estimatedHours
            ? parseInt(taskFormData.estimatedHours)
            : null,
          ...
          {restrictAccess && (
            <div className="p-4 rounded-md bg-destructive/10 border border-destructive/20 flex items-center gap-2">
              ...
              <p className="text-sm text-destructive font-medium">
                Access restricted: invoice overdue by 30+ days. Please settle
                dues to continue.
              </p>
            </div>
          )}
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-medium">
                            ...
                            {activity.type === "comment" &&
                              `Comment: ${activity.data.comment_text?.slice(0, 50)}...`}
                            {activity.type === "task" &&
                              `Task updated: ${activity.data.name}`}
                          </p>
                          <p className="text-xs text-muted-foreground">
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-2xl font-bold">
                      {deliverablesStats.completionRate}%
                    </p>
                  ...
                  >
                    ...
                    <DialogContent>
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          Submit a task request. It will be reviewed and
                      ...
                      <DialogFooter>
                        ...
                        <Button
                          ...
                          disabled={
                            !taskFormData.title.trim() || taskSubmitting
                          }
                        >
              ...
              <Card>
                <CardContent className="p-6">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {budgetStats.spentPercentage}% of budget
                  </p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-6">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {100 - budgetStats.spentPercentage}% available
                  </p>
                </CardContent>
              </Card>
            </div>
