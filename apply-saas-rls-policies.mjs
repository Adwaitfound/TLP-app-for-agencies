...
-- RLS Policies for magic_links (mainly service role)
DROP POLICY IF EXISTS "service_role_all_magic_links" ON magic_links;
CREATE POLICY "service_role_all_magic_links"
...
async function applyRLS() {
  console.log('🔄 Applying RLS policies for SaaS tables...\n');
  
  try {
    const { error } = await supabase.rpc('exec_sql', { sql });
    
    if (error) {
      console.log('⚠️ RPC method not available, applying via SQL directly...');
      
      // Apply the SQL directly using individual queries
    ...
    console.log('\n💡 The RLS policies need to be applied manually via Supabase SQL editor:');
    console.log('\n---SQL---');
    console.log(sql);
    console.log('---END SQL---');
  }
